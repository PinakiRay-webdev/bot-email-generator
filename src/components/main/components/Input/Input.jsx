...
const Input = () => {
  ...
  const handleFormData = async (data) => {
    ...
    if (!session) {
      toast.warning("Please create an account or log in first.", { theme: "dark" });
      return;
    }

    toast.loading("Generating form fields...", { theme: "dark", autoClose: false });
    ...
    try {
      ...
      toast.success("Form fields generated! Please fill them out.", { theme: "dark" });
  ...
  const genMail = async () => {
    ...
    if (typeof dynamicInputArray !== 'object' || !Array.isArray(dynamicInputArray)) {
        console.error("dynamicInputArray is not an array:", dynamicInputArray);
        toast.error("Error: Form fields structure is invalid.", { theme: "dark" });
        return; 
    }
  ...
  return (
    <div className="text-slate-400 mt-20 md:mt-28 px-4"> 
      ...
      <form
        ...
        className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-[90%] md:w-[80%] mx-auto mt-8" 
      >
        <fieldset
          className={`border px-3 py-2 col-span-1 md:col-span-2 rounded-md transition-colors ${
            errors.subject ? "border-red-500" : "border-zinc-600 focus-within:border-blue-500"
          }`}
        >
          <legend className={`px-1 text-sm transition-colors ${errors.subject ? "text-red-500" : "text-zinc-400"}`}>
          ...
          <div className="flex items-center">
            <input
              ...
              className={`w-full outline-none bg-transparent text-base placeholder-zinc-500 ${
                !isSubjectEditable ? "cursor-not-allowed text-gray-500" : "text-slate-300"
              }`}
              type="text"
            ...
            {isSubjectEditable && (
              isListening ? (
                <span className="text-sm text-blue-400 animate-pulse ml-2 whitespace-nowrap">Listening...</span>
              ) : (
                <button type="button" onClick={startVoiceTyping} className="text-2xl text-zinc-400 hover:text-white transition-colors ml-2 p-1 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded">
        ...
        <fieldset className={`border px-3 py-2 col-span-1 rounded-md transition-colors ${ "border-zinc-600 focus-within:border-blue-500" }`}>
          <legend className="px-1 text-sm text-zinc-400">
            Tone
          </legend>
          <select
            {...register("tone")}
            className={`w-full outline-none bg-transparent text-base ${!isSubjectEditable ? "cursor-not-allowed text-gray-500" : "text-slate-300"}`}
            disabled={!isSubjectEditable}
          >
            <option className="text-black bg-white" value="Formal">Formal</option>
            <option className="text-black bg-white" value="Informal">Informal</option>
            <option className="text-black bg-white" value="Casual">Casual</option>
            <option className="text-black bg-white" value="Professional">Professional</option>
          </select>
        </fieldset>


        {isDynamicFormVisible === 'block' && typeof(dynamicInputArray) === 'object' && Array.isArray(dynamicInputArray) && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 col-span-1 md:col-span-3 gap-4 md:gap-6 mt-4">
            {dynamicInputArray.map((key, index) => {
                ...
                const inputType = lowerKey.includes('date') ? 'date'
                                : lowerKey.includes('time') ? 'time'
                                : lowerKey.includes('email') ? 'email'
                                : lowerKey.includes('number') || lowerKey.includes('count') || lowerKey.includes('quantity') ? 'number'
                                : 'text';

                const legendText = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()).trim();

                return (
                    <fieldset
                        key={`${key}-${index}`}
                        className={`border px-3 py-2 rounded-md transition-colors ${
                            errors[key] ? "border-red-500" : "border-zinc-600 focus-within:border-blue-500"
                        }`}
                    >
                        <legend className={`px-1 text-sm transition-colors ${errors[key] ? "text-red-500" : "text-zinc-400"}`}>
                            {errors[key] ? errors[key].message : legendText}
                        </legend>
                        <input
                            {...register(key, { required: `${legendText} is required` })}
      ...
      {mail && (
        <div className="bg-zinc-800/50 backdrop-blur-sm border border-zinc-700 px-4 sm:px-6 py-6 mt-8 rounded-xl text-white w-[90%] md:w-[80%] mx-auto shadow-lg">
          <header className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-slate-300">Generated Mail</h3>
            <button
          ...
          <p className="whitespace-pre-wrap text-base leading-relaxed text-slate-300">{mail}</p>
        </div>
      )}
